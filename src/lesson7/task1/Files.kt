...
//{
//    val appearings = mutableListOf<Int>()
//    val map = mutableMapOf<String, Int>()
//    for (i in 0 until substrings.size) {
//        appearings += 0
//    }
//    val outputName = File(inputName).readLines()
//    for (i in 0 until outputName.size) {
//        for (j in 0 until substrings.size)
//            //if (substrings[j].toLowerCase() in outputName[i].toLowerCase()) appearings[j]++
//            appearings[j] += substrings[j].toLowerCase().toRegex().findAll(outputName[i].toLowerCase(), startIndex = 0).toMutableList().size
//    }
//    for (i in 0 until substrings.size) {
//        map += (substrings[i] to appearings[i])
//    }
//    return map
//}
    ...
    if ((result.size == 1) && (reducer.length + 1 <= lhvs.length)) {
        ...
        if (lhv / rhv > 9) {
            for (i in 1 until result.size) {
                ...
                lines += separate.padStart(maxOf(rest.length, reducer.length + 1), '-').padStart(lines.last().length)
                rest = (rest.toInt() - reducer.toInt()).toString() + (lhvpos % 10).toString()
                lines += if ((i == result.size - 1)) // && (lhv % rhv <= 9))
                    ((rest.toInt() / 10).toString()).padStart(lines.last().length) else
